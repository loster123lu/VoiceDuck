...
物理麦克风 ───► WASAPI capture  ─► 重采样 ─► 双声道同声 ─► 麦克风增益 ─┼─► 限幅 ─► CABLE Input
...
- 麦克风分支按帧选择幅度较大的声道并复制到左右声道，兼容只采单侧或只读取单侧的设备与通话软件；
- 播放分支通过有界缓冲形成约 350 ms 延迟，再经过实时回声保护门；
- `AudioEngineService` 独立读取已知或配置通话进程的输出峰值。超过安全阈值时，保护门持续读取并丢弃播放数据，保持至延迟队列越过远端语音；
- 播放分支与麦克风混合、限幅后送 `CABLE Input`；真实麦克风达到讲话阈值时播放分支快速衰减并平滑恢复；暂停播放声或触发回声保护时，麦克风仍保持在线；
- 分享启动先等待真实麦克风返回首个数据帧，确认采集链路建立后才允许后续默认输入切换；
...
提供驱动状态、麦克风/播放端点选择、双增益、电平、播放声暂停控制与回声保护状态。开始与停止复用一个状态按钮，不再要求额外勾选路由确认。启用自动默认麦克风后，输入跟随“系统默认设备”的通话应用无需再次手动选择；被应用单独固定的输入端点仍需在应用内修改。
